...
int main(int argc, char *argv[])
{
	...
	try {
		...
		const int histo_width = 255, histo_height = histo_width*2, histo_total = histo_width*histo_height ;
		ABuffer<int> histo_data_buffer(histo_width) ;
		
		
		int *histo_data = histo_data_buffer.get() ;
		...
		ABuffer<float_type> cumulative_frequences_buffer(total) ;
		cumulative_frequences_buffer.zero() ;
		float_type *cum_freq = cumulative_frequences_buffer.get() ;
		
		cum_freq[0] = histo_data[0] ;
		cout << cum_freq[0] ;
		for ( int i = 1 ; i < histo_width ; i++ ) {
			cum_freq[i] = histo_data[i] + cum_freq[i-1] ;
			// cout << cum_freq[i] << " " ;
		}
		...
		//ABuffer<int> equ_image_buffer(total) ;
		//int *equ_image = equ_image_buffer.get() ;
		
		for ( int i = 0 ; i < total ; i++ ) {
			equ_image[i] = cum_freq[img[i]] * 255 / total ;
			//equ_image[i] = equ_image[i] * max_pixel / 255 ;
			equ_image[i] *= 256 ;
			//cout << equ_image[i] << " " ;
		}
		
		// now generate the final image
		//normalize_int(histo_data,histo_width,0,histo_height) ;
