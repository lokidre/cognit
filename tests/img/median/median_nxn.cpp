...
int main(int argc, char *argv[])
{
	...
	try {
		...
		buffer.resize(filter_size*filter_size) ;
		
		
		median_nxn(image.get(),filtered.get(),width,height,filter_size,buffer.get()) ;

		
		image_save(filtered.get(),width,height,str_format(output_file,"%s-median-%dx%d.%s",input_file_name,filter_size,filter_size,input_file_ext)) ;
