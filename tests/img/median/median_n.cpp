...
int main(int argc, char *argv[])
{
	...
	try {
		...
		for ( int y = 0 ; y < height ; ++y, pi += width, pf += width ) {
			median_n(pi,pf,width,filter_size,buffer.get()) ;
		}

		
		image_save(filtered.get(),width,height,str_format(output_file,"%s-median-%d.%s",input_file_name,filter_size,input_file_ext)) ;
