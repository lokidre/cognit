...
{                 
  strcpy ( a_file_name, argc > 1 ? argv[1] : "dat/a_lu6" ) ;
  strcpy ( c_file_name, argc > 3 ? argv[3] : "dat/c_lu6" ) ;
  ...
  try {
    ...
    if ( m != n || m > 5 ) {
      ...
      a_stream >> a6 ;
      c6.setup(m,n) ;
      if ( !__mtx_lu_crout ( a6.p, n, c6.p ) )
        cout << "matrix A is singular" << endl ;
      c_stream << m << " " << n << endl << c6 ;

      __mtx_lu_breakup ( c6.p, n, a6.p, c6.p ) ;
      cout << a6 << endl << c6 << endl << MtxCleanup(a6*c6) << endl ;
    } else if ( m == 2 ) {
      //a_stream >> a2 ;
      //if ( !MtxInv(a2,c2) )
